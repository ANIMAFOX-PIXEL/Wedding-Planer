...
function validateUserCreate(username, email, name, password){
...
async function userCreate(req, res){
    ...
    const userErr = 
        validateUserCreate(username, email, name, password);

    if( userErr ) 
        return res.status(400).send(userErr);

    const { code, err, data } = 
        await userService.userCreate(username, email, name, password);
