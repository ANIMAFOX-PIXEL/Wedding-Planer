const { commerce, helpers } = require('faker');
...
function validateProductCreate( name, description, price, userId, tags){
    ...
    if(!tags || tags.length===0)
        return 'tags mandatory, must not be empty';
...
async function productCreate(req, res){
    ...
    const {code, err, data} = 
        await productService.productCreate(userId, name, description, price, tags, [...req.files]);
    if(err)
...
async function productRead(req, res){
    ...
    const {code, err, data} = await productService.productRead(id);
    if(err)
        return res.status(code).send(err);
...
async function productReadAll(req,res){
    const {code, err, data} = await productService.productReadAll();
    if(err)
        return res.status(code).send(err);
